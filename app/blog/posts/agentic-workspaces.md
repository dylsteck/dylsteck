...
That realization shifted my focus with Cortex. Instead of trying to build the big unified interface, what mattered more was the data layer and the SDK for agents — getting your data ready so agents can actually use it on your computer. The interface would come, but the foundation had to be right first.

Around the same time, I built [Ship](https://github.com/dylsteck/ship), a personal background coding agent inspired by [Ramp's Inspect](https://builders.ramp.com/post/why-we-built-our-background-agent). The idea: spin up a sandboxed cloud VM with Modal, clone a repo, and let an agent work on it autonomously while you do other things — then check back later. It's a Next.js dashboard on top of OpenCode where you kick off a task and messages stream in real-time. Building it taught me a lot about the local vs. cloud split — ephemeral sandboxes are great for fire-and-forget tasks, while persistent local setups are better for ongoing work. I'm sure those learnings will converge, but Ship gave me a feel for what the cloud side of agent coordination looks like in practice.
...
Each attempt taught me something. The timeline is a good unifying metaphor. Local-first data ownership matters. Performance matters more than you think. And the biggest lesson: scope it to what agents can actually do *today*, because that's moving fast enough on its own.
